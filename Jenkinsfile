pipeline {
  ...
  environment {
    ...
    BUILD_DIR      = 'dist'                
    CLOUDFRONT_ID  = 'EZ8M10K3BEIHP'       
    // ROUTE53_ZONE_ID = 'Z08434841EIHQSGBDO2G3'  
    // DOMAIN_NAME      = 'app.golabing.ai' 
    AWS_CREDENTIALS_ID = 'a9cd0d04-49fd-4ec3-8fd0-29122149b3b6'   // ✅ Jenkins credential ID
  }
  ...
  stages {
    ...
    stage('CloudFront Invalidate') {
      steps {
        withAWS(credentials: "${AWS_CREDENTIALS_ID}", region: "${AWS_REGION}") {
          sh """
            echo "♻️ Invalidating CloudFront distribution ${CLOUDFRONT_ID} ..."
      ...
      steps {
        withAWS(credentials: "${AWS_CREDENTIALS_ID}", region: "${AWS_REGION}") {
          script {
            ...
            def cfHostedZoneId = "Z2FDTNDATAQYW2" // CloudFront hosted zone ID
            def change = """
            {
  ...
  post {
    success {
      echo "✅ Deployment succeeded! URL: https://${env.DOMAIN_NAME ?: env.S3_BUCKET}"
    }
    failure {
